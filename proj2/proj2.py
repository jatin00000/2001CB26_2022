...
#This function processes files of inputed directory one by one
def octant_analysis_single_file( myFile,dirname, mod=5000):
		a = myFile.split("\\")[-1]
	#used to return all file paths that match a specific pattern. 
		...
		filename = a.split(".xlsx")[0]+"_{0}_{1}.xlsx".format(mod, t)
		Pointer2.to_excel(f'{dirname}\\{filename}', sheet_name='sheet1',index = False)
		...
		wb = openpyxl.load_workbook(f'{dirname}\\{filename}')
		...
		wb.save(f'{dirname}\\{filename}')
		os.remove(myFile)

#This function is for single file processing
#this function in current directory generate a temporary excel file during execution, named as 'Temp.xlsx'
#After providing download option, this file is deleted
def octant_analysis_For_single_file( myFile, mod=5000): 
		#Creating list for storing cells address for draw border and highlighting
		_border,_color = list(), list()
		...
		Pointer2.rename(columns = {x:'' for x in ['Dummy1', '1','-1','2','-2','3','-3','4','-4','Rank Octant 1','Rank Octant -1','Rank Octant 2','Rank Octant -2','Rank Octant 3','Rank Octant -3','Rank Octant 4','Rank Octant -4','Rank1 Octant ID', 'Rank1 Octant Name' ]}, inplace = True)
		Pointer2.rename(columns = {x:'' for x in ['T','U', 'V', 'W', 'U Avg', 'V Avg', 'W Avg','U\'=U - U avg','V\'=V - V avg','W\'=W - W avg','Octant']}, inplace = True)
		Pointer2.to_excel( "./Temp.xlsx",sheet_name = 'sheet1', index = False)
		#23 Saving all changes to output file
